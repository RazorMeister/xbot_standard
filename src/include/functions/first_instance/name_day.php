<?php
	...
	********************************/
	function name_day($cfg)
	{
		...
		$output = $query->channelEdit($cfg['channel_id'], array('channel_name' => str_replace('[NAME]', $name_day[date("m-d")], $cfg['channel_name']), 'channel_description' => ''));
		...
		if(!$output['success'])
		{
			foreach($output['errors'] as $error)
			{
				...
				{
					case 1541:
						$query->channelEdit($cfg['channel_id'], array('channel_description' => "[hr][center][img]https://i.imgur.com/c2BVGB7.png[/img][/center]\n● [size=13][B]Lista błędów[/B][/size][list][*][size=9][b]Błąd [u]nazwa kanału jest za długa[/u][/size][/list]\n".$language['function']['down_desc']));
						break;
					default: 
						break;
				}
