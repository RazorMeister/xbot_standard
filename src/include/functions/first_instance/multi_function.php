<?php
	...
	********************************/
	function multi_function($cfg)
	{
		...
		if($cfg['content']['channels_count']['enabled'])
		{
			$channels = $server_info['virtualserver_channelsonline'];
			$function = $cfg['content']['channels_count'];

			$query->channelEdit($function['channel_id'], array('channel_name' => str_replace("[CHANNELS]", $channels, $function['channel_name'])));
		}
	}
?>